...
/**
 * Nano Banana Pro — modelo disponível nesta conta.
 * Usa generateContent com responseModalities IMAGE.
 */
async function tryNanoBanana(prompt: string): Promise<string> {
  ...
  if (!img?.inlineData?.data) throw new Error(`NanaBanana: sem imagem — ${JSON.stringify(parts).slice(0, 150)}`);
